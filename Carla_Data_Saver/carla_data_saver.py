...
def conf_to_carla_transform(conf):
    loc, rot = conf.get("location", {'x': 0.0, 'y': 0.0, 'z': 0.0}), conf.get(
        "rotation", {'roll': 0.0, 'pitch': 0.0, 'yaw': 0.0})
    location = carla.Location(
        x=loc.get('x', 0.0), y=loc.get('y', 0.0), z=loc.get('z', 0.0))
    rotation = carla.Rotation(roll=rot.get('roll', 0.0), pitch=rot.get(
        'pitch', 0.0), yaw=rot.get('yaw', 0.0))
...
def calculate_sensor_step_diff(conf, actor):
    ...
    if jitter is not None:
        ...
        motion_step_dict["jitter_bound_min"] = carla.Location(
            x=-1 * jitter.get('x', 0.0), y=-1 * jitter.get('y', 0.0), z=-1 * jitter.get('x', 0.0))
        motion_step_dict["jitter_bound_max"] = carla.Location(
            x=jitter.get('x', 0.0), y=jitter.get('y', 0.0), z=jitter.get('x', 0.0))
    ...
    if start is not None and dest is not None:
        ...
        time = math.ceil(
            math.sqrt((x_dist)**2 + (y_dist)**2 + (z_dist)**2)) / speed

        x_step = x_dist / time
        y_step = y_dist / time
        z_step = z_dist / time

        motion_step_dict["linear_motion_step"] = carla.Location(
            x=x_step, y=y_step, z=z_step)
        motion_step_dict["linear_motion_time"] = time
        motion_step_dict["destination"] = carla.Location(x=dest_loc.get(
            'x', 0.0), y=dest_loc.get('y', 0.0), z=dest_loc.get('z', 0.0))
    return motion_step_dict


def step_sensor(sensor, sensor_motion_dicts, local_time_step):
    """Function to step sensors that have a specified motion configured (as per <config.
    >)
    ...
    if jitter is not None:
        ...
        if local_time_step == 1:
            sensor_motion_dict["jitter_prev"] = carla.Location(
...
def spawn_actors(conf, world, parent=None):
    ...
    for actor_conf in conf:
        ...
        logger.info(
            f"Spawned actor: {actor_list[0].type_id} with id: {actor_list[0].id}")
        ...
        for actor in actor_list:
            ...
            if derived_parent is not None and (fnmatch.fnmatch(derived_parent.type_id, "vehicle.*") or fnmatch.fnmatch(derived_parent.type_id, "walker.pedestrian.*")):
                ...
                actors[derived_parent.id] = (
                    derived_parent, derived_parent == parent, None)
                logger.info(
                    f"Found actor : {derived_parent.type_id} with id: {derived_parent.id} to enable autopilot")

        attached_actors, sensor_motion_steps = spawn_actors(
            actor_conf.get("sensors", []), world, parent=actor)
...
def conf_to_actor(conf, world, parent=None):
    ...
    if "destination_transform" not in conf:
        ...
        dest_transform = conf_to_carla_transform(
            conf.destination_transform).location

    actor = world.spawn_actor(blueprint, transform,
                              attach_to=attach_to, attachment_type=attachment)
    world.tick()
    ...
    if fnmatch.fnmatch(actor.type_id, "walker.pedestrian.*"):
        ...
        walker_controller = world.spawn_actor(
            walker_controller_bp, carla.Transform(), attach_to=actor)
        world.tick()
        walker_speed = conf.blueprint.get("speed", float(blueprint.get_attribute(
            'speed').recommended_values[1]))  # Set default value to walking speed 1.7 m/s
        logger.debug("Recommended value for walker speed: " + str(
            blueprint.get_attribute('speed').recommended_values))  # Recommended values
...
def conf_to_attach_to(conf, world, default_attach_to=None):
    ...
    if isinstance(attach_to, str):
        attach_to = wait_for_actor_with_attribute_to_spawn(
            world, "role_name", attach_to)

    return attach_to


def wait_for_actor_with_attribute_to_spawn(world, attribute_name, attribute_value):
    logger.info("Waiting for actor with role_name %s to spawn...",
                attribute_value)
    ...
    while actor is None:
        ...
        actors = list(filter(lambda actor: actor.attributes.get(
            attribute_name) == attribute_value, actors))
...
def actors_still_alive(snapshot, actor_dict):
    ...
    dead_actors = [actor_dict[id]
                   for id in set(actors_spawned) - set(actors_alive)]
...
def save_sensor_dynamic_metadata(sensor_name, sensorsnapshot, sensor):
    ...
    sensor_metadata['actorsnapshot_location'] = (
        snapshot_transform.location.x, snapshot_transform.location.y, snapshot_transform.location.z)
    sensor_metadata['actorsnapshot_rotation'] = (
        snapshot_transform.rotation.pitch, snapshot_transform.rotation.yaw, snapshot_transform.rotation.roll)
...
def save_actors_static_metadata(actors):
    ...
    for actor in actors:
        ...
        actor_data['3D_BoundingBox']['extent'] = (
...
def save_actors_dynamic_metadata(actors, worldsnapshot, world):
    ...
    for id, actor in actors.items():
        ...
        actor_data['actorsnapshot_location'] = (
            actorsnapshot_transform.location.x, actorsnapshot_transform.location.y, actorsnapshot_transform.location.z)
        actor_data['actorsnapshot_rotation'] = (
            actorsnapshot_transform.rotation.pitch, actorsnapshot_transform.rotation.yaw, actorsnapshot_transform.rotation.roll)
        ...
        if 'walker' in actor['type']:
            ...
            for bone in bones:
                ...
                bone_data['relative']['location'] = (
                    bone.relative.location.x, bone.relative.location.y, bone.relative.location.z)
                bone_data['relative']['rotation'] = (
                    bone.relative.rotation.pitch, bone.relative.rotation.yaw, bone.relative.rotation.roll)
...
def data_saver_loop(conf):
    ...
    for i in range(retry):
        try:
            ...
            #         conf.carla.townmap, reset_settings=False)
            # else:
            ...
            logger.info("Time between ticks: " + str(t_between_ticks) +
                        " and Delta seconds: " + str(world.get_settings().fixed_delta_seconds))
    ...
    try:
        ...
        logger.info(
            "Retrieved sensor motion dict after spawning all actors" + str(sensor_motion_dict))
        world.tick()
        ...
        sensor_list = list(map(lambda x: x[0], filter(
            lambda a: fnmatch.fnmatch(a[0].type_id, "sensor.*"), actor_dict.values())))
        sensor_id_to_sensor_type = {
            actor.id: actor.type_id for actor in sensor_list}
        ...
        metadata_static_file = os.path.join(
            metadata_path, 'metadata_static.json')
        ...
        with CarlaSyncMode(client, *sensor_list, tm_port=tm_port, tm_seed=conf.carla.seed, fps=fps, respawn=respawn) as sync_mode:
            while True:
                ...
                if local_frame_num > max_frames:
                    logger.info(
                        f"Max frames, {max_frames}, reached. Exiting...")
                    break
                # Advance the simulation and wait for the data.
                (_, snapshot), * \
                    sensor_data = sync_mode.tick(
                        conf.carla.sync.get("timeout", 2.0))
                ...
                if not actors_still_alive(snapshot, actor_dict):
                    ...
                    break
                logger.info("Snapshot: %s and length of sensor_data: %d",
                            snapshot, len(sensor_data))
                ...
                metadata_dynamic['actors'] = save_actors_dynamic_metadata(
                    metadata_static['actors'], snapshot, world)
                ...
                for sensor_id, data in sensor_data:
                    ...
                    path = os.path.join(
                        conf.output_dir, sensor_name, f"{local_frame_num:08}")

                    metadata_dynamic['sensors'][sensor_id] = save_sensor_dynamic_metadata(
                        sensor_name, sensor, world.get_actor(sensor_id))

                    # CARLA's default file format for saving LIDAR data is ply, which can take a lot of disk space
                    data.save_to_disk(path)
                    step_sensor(world.get_actor(sensor_id),
                                sensor_motion_dict, local_frame_num)
            ...
            for sensor_id in sensor_id_to_sensor_type.keys():
                if sensor_id_to_sensor_type[sensor_id] == "sensor.camera.rgb":
                    img_folder = os.path.join(
                        conf.output_dir, sensor_id_to_sensor_type[sensor_id] + "." + str(sensor_id) + "/*.png")
                    output_dir = os.path.join(
                        conf.output_dir, sensor_id_to_sensor_type[sensor_id] + "." + str(sensor_id) + '.gif')
                    gif_maker(img_folder, output_dir)
